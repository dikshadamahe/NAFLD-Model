...
FIXES APPLIED v2.0:
  FIX-1 : VotingClassifier moved inside unified model loop (models dict).
           No longer built "after the loop" — fully integrated as model #24.
  FIX-2 : np.str_ cosmetic artifact resolved by converting feat_names to
           a plain Python list of str at source; all downstream code uses
           clean strings with no NumPy scalar wrappers.
  FIX-3 : Analysis 6 label corrected — "AUC Advantage of Best Model"
           replaces the misleading "AUC drop" label.  Sign convention is
           also fixed: best_auc - simple_auc (always ≥ 0, labelled clearly).
=============================================================================
"""
...
def get_feature_names(preprocessor, num_cols, cat_cols):
    """
    FIX-2: Returns a plain Python list[str] — no np.str_ wrappers.
    Converting via str() at the source guarantees clean strings everywhere.
    """
...
def _extract_classifier(estimator):
    """Extract base classifier from ImbPipeline if wrapped."""
...
def _build_cv_pipeline(clf, smote_k=None):
    ...
    Preprocessing is handled separately (fit on train, transform both).
    SMOTE is inside CV so it only sees training folds — no leakage.
    """
...
#
#  FIX-1:  VotingClassifier is NO LONGER built "after the loop".
#          get_models() returns all 23 base classifiers.
#          train_all_models() builds VotingClassifier INSIDE the same loop
#          as model #24, keeping numbering and flow fully consistent.
# ═══════════════════════════════════════════════════════════════════════════
def get_base_models():
    """Return ordered dict of 23 base classifiers (all except VotingClassifier)."""
    return {
        ...
        "XGBoost":                   XGBClassifier(n_estimators=200, eval_metric="logloss", random_state=SEED, verbosity=0),
...
def train_all_models(Xtr_p, ytr, Xte_p, yte, smote_k=None):
    ...
    - Preprocessing already done outside (fit on train, transform both).
    - SMOTE applied INSIDE each CV fold via ImbPipeline (no leakage).
    - Final model trained on full SMOTE'd training data for test evaluation.

    FIX-1: VotingClassifier is built INSIDE the unified loop as model #24,
           using interim CV-AUC scores from the first 23 models.
    """
    ...
    # ── Model 24: VotingClassifier — built inside the same loop flow ───
    # FIX-1: Use interim rows to find top-3 by CV ROC-AUC, then build
    #        VotingClassifier here as the natural 24th iteration.
    ...
    return ranked, fitted, Xtr_s, ytr_s
...
def analysis_4_feature_importance(fitted, feat_names):
    ...
    FIX-2: feat_names is a plain list[str] — no np.str_ wrappers possible.
    """
    ...
    for name, pal in zip(tree_model_names, palettes):
        ...
        # FIX-2: feat_names is already plain str — index directly
    ...
    if len(importance_dict) >= 2:
        all_feats = sorted(
            {f for d in importance_dict.values() for f in d}
        )
        heat = pd.DataFrame(
...
def analysis_5_shap(fitted, Xte_p, feat_names):
    ...
    FIX-2: feat_names is a plain list[str] — DataFrame columns are clean.
    Handles both list and 3D array SHAP output (newer SHAP versions).
    """
    print(f"\n{'═'*72}\n  ANALYSIS 5 : SHAP EXPLAINABILITY\n{'═'*72}")
    ...
    # FIX-2: feat_names already plain str — columns will display cleanly
    Xsample_df = pd.DataFrame(Xsample,
                               columns=feat_names[:Xsample.shape[1]])

    try:
        explainer = shap.TreeExplainer(base_mdl)
        shap_vals = explainer.shap_values(Xsample)

        # Handle both list (old SHAP) and 3D array (new SHAP)
...
# ═══════════════════════════════════════════════════════════════════════════
def analysis_6_interpretable_model(fitted, ranked, Xtr_p, ytr,
                                    Xte_p, yte, feat_names):
    """
    Train Logistic Regression on top-5 important features only.

    FIX-2: feat_names is plain list[str]; top5_names prints cleanly.
    FIX-3: Table header and sign corrected:
           - Row label renamed "AUC Advantage of Best Model" (not "AUC drop").
           - Value = best_auc - simple_auc (always ≥ 0, clearly labelled).
           - Sign shown as (+) to indicate best model's advantage.
    Uses original preprocessed training data (Xtr_p, ytr) — not SMOTE —
    for reliable, unbiased coefficients.
    """
    print(f"\n{'═'*72}\n  ANALYSIS 6 : INTERPRETABLE SIMPLE MODEL (top 5 features)\n{'═'*72}")
    ...
    if imp is None:
        print("  ⚠  No tree model with feature_importances_ found.")
        return

    n          = min(len(feat_names), len(imp))
    top5_idx   = np.argsort(imp[:n])[::-1][:5]
    # FIX-2: feat_names[j] is already a plain str
    ...
    # FIX-3: advantage = best_auc - auc_lr (always ≥ 0)
    ...
    comp = pd.DataFrame([
        ...
        },
        {
            ...
            "ROC-AUC":  best_auc,
            "Features": "all",
            "AUC Advantage of Best Model": 0.0,
        },
    ])
...
def analysis_7_external_validation(fitted, ranked, preprocessor,
    ...
    Uses original training preprocessor with transform() only — no refit.
    """
    ...
    if not os.path.exists(ext_path):
        ...
        print(f"     Expected numerical columns  : {num_cols}")
        print(f"     Expected categorical columns: {cat_cols}")
        return
    ...
    try:
        ...
        print("  ✓ Used original training preprocessor (correct scaling)")
    except Exception:
        ext_pre = build_preprocessor(common_num, common_cat)
        Xe_p    = ext_pre.fit_transform(Xe)
        print("  ⚠  Could not use original preprocessor — fitted new one.")
    ...
    try:
        ...
        for k, v in ext_metrics.items():
            orig  = ranked.iloc[0].get(k,
                    ranked.iloc[0].get(f"Test {k}", np.nan))
...
def analysis_8_mcnemar(ranked, fitted, Xte_p, yte):
    """McNemar's test between top 2 models on the same test set."""
    print(f"\n{'═'*72}\n  ANALYSIS 8 : STATISTICAL COMPARISON (McNemar's Test)\n{'═'*72}")
...
# ═══════════════════════════════════════════════════════════════════════════
def analysis_9_save_all(ranked, fitted, Xtr_p, ytr):
    """
    Save final model retrained on original preprocessed training data.
    Uses Xtr_p (original, not SMOTE) for reliable real-world deployment.
    """
    print(f"\n{'═'*72}\n  ANALYSIS 9 : SAVE ALL RESULTS\n{'═'*72}")

    best_name = ranked.iloc[0]["Model"]
    mdl       = fitted[best_name]
    mdl.fit(Xtr_p, ytr)                       # retrain on clean data
...
def main():
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║  NAFLD Research Analysis — Extended Evaluation Suite  v2.0       ║")
    print("║  Publication-Quality for IEEE / Springer                         ║")
    print("║  Zero-bug release — all FIX-1 / FIX-2 / FIX-3 applied           ║")
    print("╚════════════════════════════════════════════════════════════════════╝\n")
    ...
    if min_class < N_FOLDS:
        raise ValueError(
            f"Minority class too small for {N_FOLDS}-fold CV "
            f"(found {min_class} samples)."
        )
    ...
    smote_k = max(1, min(5, min_class_tr - 1)) if min_class_tr > 1 else None
    if smote_k is None:
        print("[WARNING] Minority class too small for SMOTE. Running without.")

    # FIX-2: get_feature_names returns plain list[str] — no np.str_ wrappers
    ...
    ranked, fitted, Xtr_s, ytr_s = train_all_models(
    ...
    analysis_6_interpretable_model(
        fitted, ranked, Xtr_p, ytr, Xte_p, yte, feat_names)
    analysis_7_external_validation(
        fitted, ranked, preprocessor, num_cols, cat_cols)
    analysis_8_mcnemar(ranked, fitted, Xte_p, yte)
    analysis_9_save_all(ranked, fitted, Xtr_p, ytr)

    # ── Final summary ──────────────────────────────────────────────────
    best = ranked.iloc[0]
    print(f"\n{'═'*72}")
    print(f"  RESEARCH ANALYSIS COMPLETE  (v2.0 — zero bug)")
    print(f"{'═'*72}")
